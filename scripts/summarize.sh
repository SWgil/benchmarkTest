...
STEM="results/${BENCH}_$(echo "$OLLAMA_MODEL" | tr ':/' '--')_$(date +%Y%m%d)"
agentdojo-summarize --logdir "$LOGDIR" --out "$STEM.md" --csv "$STEM.csv"
{
  echo; echo "## 실행 환경"
  echo "- bench: $BENCH ($(python -c 'from agentdojo_ollama.compat import agentdojo_root; print(agentdojo_root())'))"
